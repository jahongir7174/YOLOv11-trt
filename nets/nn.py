...
class NMS(torch.autograd.Function):
    ...
    def forward(ctx,
        ...
        num = torch.randint(0, max_output_boxes, (batch_size, 1), dtype=torch.int32)
        ...
        return num, boxes, scores, labels
...
class Head(torch.nn.Module):
    ...
    def forward_trt(self, x):
        shape = x[0].shape
        b, res, b_reg_num = shape[0], [], self.ch * 4
        for i in range(self.nl):
            res.append(torch.cat((self.box[i](x[i]), self.cls[i](x[i])), 1))
        if self.shape != shape:
            self.anchors, self.strides = (x.transpose(0, 1) for x in make_anchors(x, self.stride))
            self.shape = shape
        x = [i.view(b, self.no, -1) for i in res]
        y = torch.cat(x, 2)
        boxes, scores = y[:, :b_reg_num, ...], y[:, b_reg_num:, ...].sigmoid()
        boxes = boxes.view(b, 4, self.ch, -1).permute(0, 1, 3, 2)
        boxes = boxes.softmax(-1) @ torch.arange(self.ch).to(boxes)
        boxes = self.anchors.repeat(b, 2, 1) + torch.cat([-boxes[:, :2, ...], boxes[:, 2:, ...]], 1)
        boxes = boxes * self.strides

        return NMS.apply(boxes.transpose(1, 2),
                         scores.transpose(1, 2),
                         self.iou_threshold, self.conf_threshold, self.top_k)
